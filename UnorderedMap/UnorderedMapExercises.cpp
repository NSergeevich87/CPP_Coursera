...
StringIntMap makeWordCounts(const StringVec& words) {
  // Create a new map to store the word counts in stack memory.
  StringIntMap wordcount_map;
  // Loop through the input vector of strings.
  for (const std::string& word: words)
  {
    // for each word, a key should be created in the StringIntMap.
...
int lookupWithFallback(const StringIntMap& wordcount_map, const std::string& key, int fallbackVal) {
  ...
  {
    // If the key exists, return the mapped value.
...
int memoizedLongestPalindromeLength(LengthMemo& memo, const std::string& str, int leftLimit, int rightLimit, timeUnit startTime, double maxDuration) {
  ...
  if (memo.count(pairKey)) {
    ...
    // return what's already stored at this key in the map.
    return memo[pairKey];
  }
  ...
  // We've calculated this subproblem for the first time in this case.
  // We should store the result in the memoization table before returning it.

  // Store the result in the memoization table before returning it.
